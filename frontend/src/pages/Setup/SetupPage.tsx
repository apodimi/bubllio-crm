...
import {
  ...
  ReviewStep,
  WorkspaceStep,
} from '../../features/setup/components/SetupSteps'
...
export function SetupPage({ onComplete }: { onComplete: () => void }) {
  ...
  async function next() {
    const fields =
      step === 0 ? adminFields : step === 1 ? workspaceFields : smtpEnabled ? smtpFields : []
  ...
  async function sendTestEmail() {
    ...
    if (!recipient.success) {
      form.setError(
        'smtp_recipient',
        { message: recipient.error.issues[0].message },
        { shouldFocus: true },
      )
      return
    }
        ...
        >
          ...
          <Stack sx={{ p: { xs: 3, sm: 5, md: 6 }, minWidth: 0 }}>
            ...
            >
              ...
              {step === 2 && (
                <EmailStep
                  testEmail={() => void sendTestEmail()}
                  resetTest={() => emailTest.reset()}
                  testResult={emailTest}
                />
              )}
              {step === 3 && (
                <ReviewStep values={form.getValues()} testSent={emailTest.isSuccess} />
              )}
              <Box sx={{ flex: 1 }} />

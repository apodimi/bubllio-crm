...
export type WorkspaceMember = { id: string; username: string; email: string; role: InvitationRole | 'owner' }
export type InvitationPreview = { email: string; organization_name: string; role: InvitationRole; expires_at: string }
...
export const invitationService = {
  list: (organizationId: string, signal?: AbortSignal) => request<Invitation[]>(`${organizationPath(organizationId)}invitations/`, { signal }),
  members: (organizationId: string, signal?: AbortSignal) => request<WorkspaceMember[]>(`${organizationPath(organizationId)}members/`, { signal }),
  ...
  preview: (token: string, signal?: AbortSignal) => request<InvitationPreview>(invitationPath(token), { signal }),

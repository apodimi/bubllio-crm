...
export const setupSchema = z.object({
  ...
  organization_slug: z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, 'Use lowercase letters, numbers and hyphens only.'),
  ...
  for (const [field, label] of requiredSmtpFields) {
    if (!values[field].trim()) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: `${label} is required.` })
    }
  }
  if (!z.string().email().safeParse(values.smtp_from_email).success) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['smtp_from_email'], message: 'Enter a valid sender email address.' })
  }
  const port = Number(values.smtp_port)
  if (!/^\d+$/.test(values.smtp_port) || !Number.isInteger(port) || port < 1 || port > 65535) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['smtp_port'], message: 'Enter a port from 1 to 65535.' })
  }
})
...
export const setupDefaults: SetupFormValues = {
  ...
  smtp_enabled: false, smtp_name: 'Primary', smtp_from_email: '',
  smtp_host: '', smtp_port: '587', smtp_username: '', smtp_password: '',
  smtp_use_ssl: false, smtp_recipient: '',
}

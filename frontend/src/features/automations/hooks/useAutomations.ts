...
export const automationKeys = {
  byOrganization: (organizationId: string) => ['organizations', organizationId, 'automations'] as const,
  runs: (organizationId: string) => ['organizations', organizationId, 'runs'] as const,
}

const automationsQuery = (organizationId: string) => queryOptions({
  queryKey: automationKeys.byOrganization(organizationId),
  queryFn: ({ signal }) => automationService.list(organizationId, signal),
})

const automationRunsQuery = (organizationId: string) => queryOptions({
...
export const useAutomationRuns = (organizationId: string) => useQuery(automationRunsQuery(organizationId))

...
import { organizationPath } from '../features/organizations/services/organizationService'
...
it('isolates resource keys and URL segments by tenant', () => {
  expect(companyKeys.byOrganization('one')).not.toEqual(companyKeys.byOrganization('two'))
  expect(contactKeys.byOrganization('one')).not.toEqual(companyKeys.byOrganization('one'))

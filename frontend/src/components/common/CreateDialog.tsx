...
export function CreateDialog({ title, path, fields, invalidate, onClose }: {
  ...
  return <Dialog open onClose={mutation.isPending ? undefined : onClose} fullWidth maxWidth="sm">
    <form onSubmit={submit}>
      <DialogTitle>{title}</DialogTitle>
      <DialogContent><Stack spacing={2.5} sx={{ pt: 1 }}>
        {mutation.isError && <Alert severity="error">{mutation.error.message}</Alert>}
        {fields.map(field => <TextField key={field.name} label={field.label} required={field.required}
          name={field.name} type={field.type || 'text'} select={!!field.options} fullWidth
          value={values[field.name] ?? ''} disabled={mutation.isPending}
      ...
      <DialogActions sx={{ p: 3 }}><Button onClick={onClose} disabled={mutation.isPending}>Cancel</Button>
        <Button type="submit" variant="contained" disabled={mutation.isPending}>{mutation.isPending ? 'Saving…' : 'Create'}</Button>
      </DialogActions>
    </form>
  </Dialog>
}
